		...
		void *data)
{
	...
	if (rwrite(&crc, sizeof(u64), 1, ciphertext) != 1)
		goto exit;

	/* write len_enc to license file */
	if (rwrite(&len_enc, sizeof(size_t), 1, ciphertext) != 1)
		goto exit;

	/* write user encrypted data to license file */
	if (rwrite(buf, sizeof(char), len, ciphertext) != len)
		goto exit;
		...
		char *file_name, char **buf, size_t *len)
{
	...
	if (rread(&crc, sizeof(u64), 1, ciphertext) != 1)
		goto exit;
	...
	if (rread(&_len, sizeof(size_t), 1, ciphertext) != 1)
		goto exit;
	...
	if (rread(_buf, sizeof(char), _len, ciphertext) != _len ||
			0 < rread(&c, sizeof(char), 1, ciphertext)) {
		goto exit;
	}
