	...
	rsa_stream_t **ciphertext, int *is_full)
{
	...
	if (!(*ciphertext = ropen(&stream_init_ciphertext))) {
		rsa_key_close(*key);
	...
	if (!is_encryption_info_only) {
		...
		if (!(is_enable = is_fwrite_enable(newfile_name)) ||
			!(*plaintext = ropen(&stream_init_plaintext))) {
			rsa_key_close(*key);
			rclose(*ciphertext);
			if (is_enable)
		...
		rsa_stream_t *plaintext)
{
	...
	do {
		...
		len = rread(buf, sizeof(char), buf_len, ciphertext);
		for (i = 0; len && i < (len-1)/sizeof(u64) + 1; i++)
			xor_buf[i] ^= RSA_RANDOM();
		rwrite(buf, sizeof(char), len, plaintext);
		rsa_timeline_update();
	}
	while (len == buf_len);
	rsa_timeline_uninit();
		...
		rsa_stream_t *plaintext)
{
	...
	do {
		...
		for (i = 0; i < ct_buf_len && len < file_size; i++) {
			...
			len += rwrite(&ct_buf[i].arr, sizeof(char),
