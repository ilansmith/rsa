	...
	rsa_stream_t **ciphertext, int is_full)
{
	...
	if (!(*plaintext = ropen(&stream_init))) {
		rsa_key_close(*key);
...
int rsa_encrypt_quick(void)
{
	...
	do {
		...
		len = rread(buf, sizeof(char), buf_len, plaintext);
		for (i = 0; len && i < (len-1)/sizeof(u64) + 1; i++)
			xor_buf[i] ^= RSA_RANDOM();
		rwrite(buf, sizeof(char), len, ciphertext);
		rsa_timeline_update();
	}
	while (len == buf_len);
	rsa_timeline_uninit();
...
int rsa_encrypt_full(void)
{
	...
	do {
		...
		len = rread(pt_buf, sizeof(char), pt_buf_len, plaintext);
